/* Vendored QR-code encoder (byte mode, auto version, level M), ported from the public-domain kazuhikoarase algorithm. Isolated from the app so it stays a self-contained unit.
   Verified end-to-end: output decodes with zxing-cpp across versions 4/8/13. */
/* ---- share: QR code + copy link ----
   Tiny self-contained QR encoder (byte mode, auto version, level M) so
   "Share" works with no network call, same offline-first spirit as the
   floor plan. Ported from the public-domain kazuhikoarase QR algorithm. */
const QRMath={EXP_TABLE:new Array(256),LOG_TABLE:new Array(256)};
(function(){for(let i=0;i<8;i++)QRMath.EXP_TABLE[i]=1<<i;
...
QRMath.glog=n=>{if(n<1)throw new Error('glog('+n+')');return QRMath.LOG_TABLE[n];};
QRMath.gexp=n=>{while(n<0)n+=255;while(n>=256)n-=255;return QRMath.EXP_TABLE[n];};
...
QRPolynomial.prototype={
  get(i){return this.num[i];},getLength(){return this.num.length;},
  multiply(e){const num=new Array(this.getLength()+e.getLength()-1).fill(0);
...
const QRUtil={
  ...
  getErrorCorrectPolynomial(len){let a=new QRPolynomial([1],0);for(let i=0;i<len;i++)a=a.multiply(new QRPolynomial([1,QRMath.gexp(i)],0));return a;},
  getLengthInBits(mode,type){
    if(type<10)return mode===4?8:(mode===1?10:9);
  ...
  getLostPoint(qr){
    ...
    for(let row=0;row<n-1;row++)for(let col=0;col<n-1;col++){
      let cnt=0;if(qr.isDark(row,col))cnt++;if(qr.isDark(row+1,col))cnt++;if(qr.isDark(row,col+1))cnt++;if(qr.isDark(row+1,col+1))cnt++;
    ...
    let dark=0;for(let col=0;col<n;col++)for(let row=0;row<n;row++)if(qr.isDark(row,col))dark++;
    lost+=Math.abs(100*dark/n/n-50)/5*10;
    return lost;}
};
function QR8bitByte(data){
  this.mode=4;this.data=data;const bytes=[];
  for(let i=0;i<data.length;i++){const code=data.charCodeAt(i);
    if(code>0x10000){bytes.push(0xF0|((code&0x1C0000)>>>18),0x80|((code&0x3F000)>>>12),0x80|((code&0xFC0)>>>6),0x80|(code&0x3F));}
    else if(code>0x800){bytes.push(0xE0|((code&0xF000)>>>12),0x80|((code&0xFC0)>>>6),0x80|(code&0x3F));}
    else if(code>0x80){bytes.push(0xC0|((code&0x7C0)>>>6),0x80|(code&0x3F));}
    else bytes.push(code);}
  this.parsedData=bytes;
}
QR8bitByte.prototype={getLength(){return this.parsedData.length;},write(buf){for(const b of this.parsedData)buf.put(b,8);}};
...
QRRSBlock.getRSBlocks=function(typeNumber,ec){
  /* table columns are ordered [L,M,Q,H]; the QR spec's format-info values for
     those levels are L=1,M=0,Q=3,H=2 (used as-is in setupTypeInfo), so they
     must be translated to the table's column order here. */
  const offset={1:0,0:1,3:2,2:3}[ec];
  if(offset===undefined)throw new Error('bad ec level');
  const table=QRRSBlock.RS_BLOCK_TABLE[(typeNumber-1)*4+offset];
  if(!table)throw new Error('bad rs block');
  const list=[];
  for(let i=0;i<table.length/3;i++){const count=table[i*3],total=table[i*3+1],dc=table[i*3+2];for(let j=0;j<count;j++)list.push(new QRRSBlock(total,dc));}
  return list;
};
function QRCodeModel(typeNumber,ec){this.typeNumber=typeNumber;this.ec=ec;this.modules=null;this.moduleCount=0;this.dataCache=null;this.dataList=[];}
...
QRCodeModel.prototype={
  ...
  getBestMaskPattern(){let minLost=0,pattern=0;
    for(let i=0;i<8;i++){this.makeImpl(true,i);const lost=QRUtil.getLostPoint(this);if(i===0||minLost>lost){minLost=lost;pattern=i;}}
    return pattern;},
  ...
  setupTypeInfo(test,maskPattern){
    const data=(this.ec<<3)|maskPattern;const bits=QRUtil.getBCHTypeInfo(data);
  ...
  createData(){
    const rsBlocks=QRRSBlock.getRSBlocks(this.typeNumber,this.ec);
    const buffer=new QRBitBuffer();
    for(const data of this.dataList){
      buffer.put(data.mode,4);buffer.put(data.getLength(),QRUtil.getLengthInBits(data.mode,this.typeNumber));data.write(buffer);}
    let totalDataCount=0;for(const b of rsBlocks)totalDataCount+=b.dataCount;
    if(buffer.getLengthInBits()>totalDataCount*8)throw new Error('code length overflow');
    if(buffer.getLengthInBits()+4<=totalDataCount*8)buffer.put(0,4);
    while(buffer.getLengthInBits()%8!==0)buffer.putBit(false);
    while(true){
      if(buffer.getLengthInBits()>=totalDataCount*8)break;buffer.put(QRCodeModel.PAD0,8);
      if(buffer.getLengthInBits()>=totalDataCount*8)break;buffer.put(QRCodeModel.PAD1,8);}
...
QRCodeModel.createBytes=function(buffer,rsBlocks){
  ...
  for(let r=0;r<rsBlocks.length;r++){
    ...
    for(let i=0;i<ecdata[r].length;i++){const mi=i+modPoly.getLength()-ecdata[r].length;ecdata[r][i]=mi>=0?modPoly.get(mi):0;}}
  let totalCodeCount=0;for(const b of rsBlocks)totalCodeCount+=b.totalCount;
...
function qrEncode(text){
  for(let t=1;t<=40;t++){
    const qr=new QRCodeModel(t,0);qr.addData(text);
    try{qr.make();return qr;}catch(e){if(!/overflow/i.test(String(e.message)))throw e;}
  }
  throw new Error('QR: data too long');
}
function drawQR(canvas,text){
  const qr=qrEncode(text);const n=qr.getModuleCount();const quiet=4;
  const cell=Math.max(3,Math.floor(216/(n+quiet*2)));
